--The detectCheckpoint.lua is part of the "PcsxrrEncodeWofkflow Project v4"
--Run this script with the TAS sound plugin 0.2
...
module("detectCheckpoint",package.seeall)
...
function writeHashframe(lineInfo)
...
function getHashedScreenshotFramePos()
	line = nil;
	lineCount = 0;
	for file in lfs.dir(screenshotFolder) do
		if lfs.attributes(file,"mode") ~= "directory" and file:find(hashFileName) ~= nil then
...
function checkSyncClipBoardUpdate()
...
function fixDesyncFrame(desyncFound)
